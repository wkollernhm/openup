        ...
        if ($results != null && is_array($results)) {
            ...
            if (intval($results['number_of_results_returned']) > 0) {
                ...
                foreach ($results['results'] as $result) {
                    ...
                    $name = Yii::app()->NameParser->clean($result['name']);
                    $score = (strcasecmp($name, $term) == 0) ? 100 : 0;
                    ...
                    if (isset($result['common_names'])) {
                        ...
                        foreach ($result['common_names'] as $common_name) {
                            ...
                            if (strpos($common_name['name'], "?") !== FALSE || strpos($common_name['name'], "Ã") !== FALSE) {
                                continue;
                            }
                            ...
                            // add source database
                            if (isset($result['source_database'])) {
                                $references[] = $result['source_database'];
                            }
